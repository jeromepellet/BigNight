...
# --- 1. PARAMÈTRES DE PONDÉRATION (ADAPTÉS) ---
W_SEASON    = 0.10  # Réduit selon ta demande
W_TEMP_8H   = 0.25      
W_FEEL_2H   = 0.20      
W_RAIN_8H   = 0.15      
W_RAIN_CURR = 0.25  # Augmenté à 25% pour donner la priorité à l'événement réel
W_LUNAR     = 0.05  
...
def calculate_migration_probability(temp_8h_avg, feel_2h, rain_8h_total, rain_curr, month, dt):
    # Normalisation des facteurs (Seuil de montée à 5°C pour plus de prudence)
    ...
    # --- PÉNALITÉ SÉCHERESSE ---
    # Si pas de pluie actuelle ET sol sec (moins de 0.5mm sur les 8 dernières heures)
    # On réduit le score de 80% pour éviter l'optimisme lié à la seule chaleur
    if rain_curr < 0.1 and rain_8h_total < 0.5:
        score *= 0.2

    # --- KILL-SWITCH FROID ---
    # Relevé à 4.0°C car la migration est cliniquement négligeable en dessous en Suisse
    if feel_2h < 4.0:
        score = 0
...
st.caption("Modèle Strict : MétéoSuisse (ICON-CH) | Kill-switch 4°C | Pénalité Sécheresse")
    ...
    if 'hourly' not in data:
        ...
        for d in sorted(df['time'].dt.date.unique()):
            ...
            for idx, row in night_df.iterrows():
                i = int(idx)
                # Calcul de la pluie cumulée sur les 8h précédentes
        ...
        if tonight_res:
            st.markdown(f"""
                <div style="padding:20px; border-radius:10px; border-left: 10px solid {tonight_res['Color']}; background:rgba(0,0,0,0.05); margin-bottom:20px;">
                    ...
                    <p style="margin:0;">Pic de probabilité : <b>{tonight_res['Score']}%</b> à <b>{tonight_res['Heure Opt.']}</b> | Source : ICON-CH</p>
                </div>
